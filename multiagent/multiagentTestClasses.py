...
def emitPublicTree(tree, testDict):
    testDict["num_agents"] = tree.numAgents
    testDict["state_state"] = tree.startState.state
    testDict["win_states"] = " ".join(sorted(list(tree.winStates)))
    testDict["lose_states"] = " ".join(sorted(list(tree.loseStates)))
    testDict["successors"] = "\n".join(["%s %s %s" % (state, action.strip("_"), nextState) for state, action, nextState in tree.successors])
    testDict["evaluation"] = "\n".join(["%s %s" % (state, tree.evaluation[state]) for state in sorted(tree.evaluation.keys())])
...
class PacmanGameTreeTest(testClasses.TestCase):
    ...
    def createPublicVersion(self):
        self.testDict['layout'] = testClasses.invertLayout(self.testDict['layout'])
        self.layout_text  = self.testDict['layout']
    # END SOLUTION NO PROMPT
...
class GraphGameTreeTest(testClasses.TestCase):
    ...
    # BEGIN SOLUTION NO PROMPT
    def createPublicVersion(self):
        emitPublicTree(self.problem, self.testDict)
        return True
    # END SOLUTION NO PROMPT
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        # maxPoints should just be the value as initialized in testClasses.py, we're not assigning just 1 point per threshold
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            if passed:
                if name == "average score":
                    if self.agentName == "ReflexAgent":
                        ...
                        raise ValueError("Must have either ReflexAgent or Expectimax Agent")
                    ...
                    raise ValueError("name must be 'average score', 'games not timed out', or 'wins'")
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" % (value, name, points, subtest_maxPoints))
